...
#if ACTIVATION_FP == 1
            arith_activation_t out_aux;
            out_aux.bin = output[idx * output_size_ + feature_idx];
            out_aux.fp += float(bias[feature_idx]);
            output[idx * output_size_ + feature_idx] = out_aux.bin;
#else
