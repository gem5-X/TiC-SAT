...
int main(){
    ...
    uint32_t* m2 = new uint32_t [D_MODEL / KERNEL_DIM]();
...
//    }
    ...
    if (format == Format::META_DATA){
        auto testDense = new Dense(D_Q, D_MODEL, query_kernel, (uint32_t *)(m1), (const uint32_t*)(m2), format);
        testDense->compute(D_SEQ, tensor_in, out);
    }else{
        auto testDense = new Dense(D_Q, D_MODEL, query_kernel, query_flag, &hidden_flag, format);
        testDense->compute(D_SEQ, tensor_in, out);
    }
